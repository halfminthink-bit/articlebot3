...
# 実行方法: python wordpress/wp-auto-hukugyo.py
# 下書き保存: python wp-auto_new_columns.py --draft
...
def main():
    # コマンドライン引数の設定
    parser = argparse.ArgumentParser(description='WordPress自動投稿ツール（新列構成版）')
    ...
    for post in posts:
        try:
            ...
            title, content = get_document_content(docs_service, post['doc_url'])
            print(f"  タイトル: {title}")
