...
# 確認なしで即削除
python wordpress/wp-auto-delete.py --confirm
"""
import os
import sys
import pathlib
...
# H: 投稿日時
# I: WordPress記事URL
def get_posts_to_delete(sheets_service):
    ...
    for i, row in enumerate(values, start=2):
        ...
        # 投稿済み（G列が「済」）かつ、WordPress記事URL（I列）がある記事のみ
        if posted and posted.lower() == '済' and wp_url and wp_url.strip():
            posts.append({
                'row_number': i,
...
# WordPress REST APIで削除
def delete_post_rest(site_url, username, app_password, post_id):
    if not site_url:
    ...
    try:
        # force=true でゴミ箱を経由せず完全削除
        r = requests.delete(api_url, headers=headers, params={'force': True}, timeout=30)
...
# XML-RPCで削除
def delete_post_xmlrpc(site_url, username, app_password, post_id):
    if not site_url:
        return False, "エラー: site_url が空です"
    ...
    try:
        ...
        blog_id = 0
        result = s.wp.deletePost(blog_id, username, app_password, post_id)
        if result:
            return True, "削除成功"
        else:
            return False, "削除失敗（XML-RPC）"
...
# スプレッドシートを更新（削除後、G列を空に、H列に削除日時、I列を空に）
def update_spreadsheet_after_delete(sheets_service, row_number):
    sheet = sheets_service.spreadsheets()
    delete_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    values = [['', f'削除済 {delete_time}', '']]
    body = {'values': values}
    sheet.values().update(
        spreadsheetId=SPREADSHEET_ID,
...
# メイン処理
def main():
    parser = argparse.ArgumentParser(description='WordPress記事削除ツール')
    parser.add_argument('--confirm', action='store_true',
                        help='確認なしで削除を実行')
    args = parser.parse_args()
    
    print("WordPress記事削除ツールを開始します...")
    auth = GoogleAuth()
    sheets_service = auth.build_service('sheets', 'v4')

    posts = get_posts_to_delete(sheets_service)
    if not posts:
        print("削除対象の記事がありません。")
        return
    ...
    if not args.confirm:
        print("\n本当に削除しますか？")
        response = input("削除する場合は 'yes' と入力してください: ")
        if response.lower() != 'yes':
            print("削除をキャンセルしました。")
            return
    
    print(f"\n{len(posts)}件の記事を削除します...\n")
    success_count = 0
    failed_count = 0

    for post in posts:
        try:
            print(f"処理中: {post['title']}")
            print(f"  URL: {post['wp_url']}")
            ...
            if ok:
                print(f"  ✓ 削除成功\n")
                update_spreadsheet_after_delete(sheets_service, post['row_number'])
                success_count += 1
            else:
                print(f"  ✗ 削除失敗: {result}\n")
                failed_count += 1
