...
class PhilosophyBotSEO:
    ...
    def create_and_post_article(
        ...
        status: str = 'draft',
        generate_ai_thumbnail: bool = True
    ) -> dict:
        """
        SEO最適化された記事を生成してWordPressに投稿
        ...
        Args:
            ...
            generate_ai_thumbnail: AI画像生成でアイキャッチを作成するか
        ...
        # HTMLを保存（デバッグ用）
        html_path = os.path.join(self.output_dir, f'{philosopher}_article_seo.html')
        with open(html_path, 'w', encoding='utf-8') as f:
            f.write(full_html)
        print(f"✅ HTML保存: {html_path}")
        
        # 3. アイキャッチ画像を生成（AI画像生成を使う場合）
        thumbnail_id = None
        if generate_ai_thumbnail:
            print("\n3. AI画像生成でアイキャッチを作成中...")
            print("   注意: この機能を使うには generate_image ツールが必要です")
...
def main():
    ...
    result = bot.create_and_post_article(
        ...
        status='draft',
        generate_ai_thumbnail=False  # 今回はスキップ
    )
